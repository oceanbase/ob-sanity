...
DEP_FILE="ob-sanity.${OS_TAG}.deps"
...
do
  ...
  if [[ "$pkg" == *"obdevtools-llvm"* || "$pkg" == *"devdeps-gtest"* ]]; then
    if [[ "$line" =~ cxxabi=([01]) ]]; then
      cxxabi="${BASH_REMATCH[1]}"
      if [ "$cxxabi" -ne "$CXX_ABI" ]; then
        continue
      fi
    fi
  fi
